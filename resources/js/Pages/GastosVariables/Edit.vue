...
const props = defineProps({
    gastoVariable: Object,
    categorias:    Array,
})

const form = useForm({
    descripcion:  props.gastoVariable.descripcion,
    monto:        props.gastoVariable.monto,
    fecha_gasto:  props.gastoVariable.fecha_gasto,
    categoria:    props.gastoVariable.categoria ?? '',
    comprobante:  props.gastoVariable.comprobante ?? '',
})

function submit() {
    form.put(route('gastos-variables.update', props.gastoVariable.id))
}
</script>

<template>
    <AuthenticatedLayout>
        <template #header>📊 Editar Gasto Variable</template>

        <div class="max-w-xl mx-auto">
            <div class="bg-white rounded-xl shadow p-6">
                <h2 class="text-xl font-bold text-gray-800 mb-6">Editar Gasto Variable</h2>
